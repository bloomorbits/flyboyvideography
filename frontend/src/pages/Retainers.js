          ...
          {rows.map((s, i) => (
            <Card key={s.id} className="card-hover rise p-7" style={{ animationDelay: `${i * 50}ms` }} data-testid={`retainer-card-${i}`}>
              ...
              <p className="mt-4 font-display text-4xl font-extrabold tracking-tighter text-accent">
                {fmtMoney(s.monthly_price)}<span className="text-base font-medium text-zinc-500">/mo</span>
              </p>
              <div className="mt-6 grid grid-cols-3 gap-4 border-t border-line pt-5 text-sm">
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Videos / mo</p>
                  <p className="mt-1 text-zinc-300">{s.videos_per_month}</p>
                </div>
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Started</p>
                  <p className="mt-1 text-zinc-300">{fmtDate(s.started_on)}</p>
                </div>
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-500">Renews</p>
                  <p className="mt-1 text-zinc-300">{fmtDate(s.renews_on)}</p>
                </div>
              </div>
            </Card>
          ))}
