    ...
    <div data-testid="invoices-page">
      ...
      {rows.length === 0 ? (
        ...
        <Card>
          ...
          {rows.map((inv, i) => (
            <div key={inv.id} className="rise grid grid-cols-12 items-center gap-4 border-b border-dune px-6 py-4 last:border-b-0 hover:bg-sand" style={{ animationDelay: `${i * 40}ms`, transition: "background-color 0.15s ease" }} data-testid={`invoice-row-${i}`}>
              <span className="col-span-3 font-mono text-sm font-bold text-ink">{inv.invoice_number}</span>
              ...
              <span className="col-span-2 text-sm text-ink/60">{fmtDate(inv.issued_on)}</span>
              <span className="col-span-2 text-sm text-ink/60">{fmtDate(inv.due_on)}</span>
